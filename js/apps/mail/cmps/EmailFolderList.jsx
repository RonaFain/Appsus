  ...
  return (
    <section className="email-folder-list">
      <ul onClick={() => onSetCriteria({status: event.target.dataset.value})}>
        ...
        <li className={activeStatus === 'inbox' ? 'my-active': ''} data-value="inbox">
        Inbox<span>{` ( ${emailService.getAmoutEmailsByFilter('stauts', 'draft', false)} )`}</span></li>
        ...
        <li className={activeStatus === 'draft' ? 'my-active': ''} data-value="draft">
        Draft<span>{` ( ${emailService.getAmoutEmailsByFilter('status', 'draft')} )`}</span></li>
      </ul>
    </section>
  )
}
